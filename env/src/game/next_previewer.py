...
class next_previewer(object):
    ...
    def render(self, image, topleft):
        ...
        if self.dirty:
            ...
            pos = (topleft[0] + self.display_pos[0] + self.vertical_border.get_width(),
                   topleft[1] + self.display_pos[1] + self.horizontal_border.get_height())
    ...
    def render_all(self, image, topleft):
        ...
        image.blit(self.horizontal_border, offset((self.vertical_border.get_width(),
                                                   self.vertical_border.get_height() - self.horizontal_border.get_height())))
        image.blit(self.vertical_border,
                   offset((self.vertical_border.get_width() + self.horizontal_border.get_width(), 0)))
        # draw the background
        image.blit(self.background, offset((self.vertical_border.get_width(), self.horizontal_border.get_height())))
        # draw the text
        image.blit(self.header, offset((PREVIEWER_MARGINS + self.vertical_border.get_width(),
                                        PREVIEWER_MARGINS + self.horizontal_border.get_height())))
