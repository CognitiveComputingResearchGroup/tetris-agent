...
class score_display(object):
    ...
    def render(self, image, topleft):
        ...
        if self.dirty:
            ...
            # draw
            toppos = (self.vertical_border.get_width() + SCORE_MARGINS + topleft[0],
                      self.horizontal_border.get_height() + SCORE_MARGINS + topleft[1])
            self.previous_scorerect = image.blit(self.scoreimg, toppos)
            self.previous_linesrect = image.blit(self.linesimg,
                                                 (toppos[0], toppos[1] + self.scoreimg.get_height() + SCORE_DIVISION))
